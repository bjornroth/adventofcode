...
void draw_screen(std::map<std::pair<int, int>, int> &grid, int max_x, int max_y,
                 int score) {
  system("clear");
...
int main(int argc, char *argv[]) {
  ...
  while (state != Intcode::kHalted) {
    ...
    if (state == Intcode::kBlocked) {
      ...
      // "AI"
      if (ball_x >= 0 && paddle_x >= 0 && ball_dir != 0) {
        paddle_ideal = ball_x + (paddle_y - 1 - ball_y) * ball_dir;
        if (paddle_x < paddle_ideal)
          input = RIGHT;
        else if (paddle_x > paddle_ideal)
          input = LEFT;
      }
      in.push_back(input);
    }

    draw_screen(grid, max_x, max_y, score);
  }
