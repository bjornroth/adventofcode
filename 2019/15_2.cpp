...
int fill_oxygen(Coord &pos, Map &map) {
  static int rowNum[] = {-1, 0, 0, 1};
  static int colNum[] = {0, -1, 1, 0};
  std::vector<Coord> oxygen;
  int count = 0;

  oxygen.push_back(pos);
  map[pos] = OXYGEN;

  while (!oxygen.empty()) {
    std::vector<Coord> next_oxygen;
    for (const auto &pt : oxygen) {
      for (int i = 0; i < 4; i++) {
        Coord test_pos;
        test_pos.first = pt.first + colNum[i];
        test_pos.second = pt.second + rowNum[i];
        if (map.find(test_pos) != map.end() && map[test_pos] == VISITED) {
          map[test_pos] = OXYGEN;
          next_oxygen.push_back(test_pos);
        }
      }
    }
...
int main(int argc, char* argv[]) {
  ...
  std::cout << fill_oxygen(robot_pos, robot_map) << std::endl;
