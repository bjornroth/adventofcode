...
int run_amp_chain(const Memory &memory, const std::vector<int> &phases) {
  Intcode amp_a(memory);
  Intcode amp_b(memory);
  Intcode amp_c(memory);
  Intcode amp_d(memory);
  Intcode amp_e(memory);

  IO a_b, b_c, c_d, d_e, e_a;
  a_b.push_front(phases[1]);
  b_c.push_front(phases[2]);
  c_d.push_front(phases[3]);
  d_e.push_front(phases[4]);
  e_a.push_front(phases[0]);
  e_a.push_back(0);
  ...
  while (!halted) {
    ...
    amp_c.run(b_c, c_d);
    amp_d.run(c_d, d_e);
    halted = (amp_e.run(d_e, e_a) == Intcode::kHalted);
  }
